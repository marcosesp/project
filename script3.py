...
import os, re, json, time, hashlib, pwd, socket, subprocess
...
def confirmar_delete(path):
    ...
    data = pending_delete.pop(path, None)
...
def process_event(lines):
    ...
    syscall = next(
        (x for x in lines if "type=SYSCALL" in x),
        None
    )

    cwd_line = next(
        (x for x in lines if "type=CWD" in x),
        None
    )
    ...
    for line in lines:
        ...
        if (name.startswith(".") and ".sw" in name) or \
           name.endswith("~") or \
           name.isdigit():
            continue

        paths.append((path, nametype))
    ...
    for path, nametype in paths:
        ...
        if nametype == "DELETE":
            ...
            # Esperamos antes de declararlo eliminado.
            # Vim normalmente lo vuelve a crear.
            timer = Timer(
                1.5,
                confirmar_delete,
                args=[path]
        ...
        # ==================================================
            ...
            # Si había un DELETE pendiente de la misma ruta,
            # realmente fue una modificación de Vim.
